...
test("server-renders the finished Lexora landing page", async () => {
  ...
  assert.match(html, /Beta\s*(?:<!-- -->)?v4\.1\.0-beta\.4/);
...
test("publishes robots, sitemap, and web app manifest", async () => {
  ...
  assert.equal(betaVersion.version, "4.1.0-beta.4");
  assert.equal(betaVersion.build, 32);
  assert.match(betaVersion.verifiedDownloads.android.filename, /v4\.1\.0-beta\.4\.apk$/);
